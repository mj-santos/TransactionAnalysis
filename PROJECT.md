...
- 5 collapsible card sections following standard collapsible panel pattern:
  ...
  - **Data Health**: Dashboard with 6 metrics (uncategorized txns, unreviewed txns, merchants without category, no merchant match, pending duplicates, orphaned categories); each metric links to relevant page; "Merchants without a category" metric expands an inline panel with search, category assignment, and "Show categorized too" toggle (`loadHealthUncategorized()` → `GET /merchant-categories/uncategorized`); orphaned categories metric shows "Fix Now" button that runs full `POST /normalize/apply` with proper job polling (button disables during run, re-enables on completion/failure) ~~(BUG-29: previously used a 2s timeout instead of polling, and `batch_renormalize` wrote to wrong column)~~
